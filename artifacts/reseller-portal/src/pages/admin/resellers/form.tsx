...
import { createResellerSchema } from "@/lib/schemas";
import { PROVINCES } from "@/lib/utils";
import { useAdminCreateReseller, getAdminGetResellersQueryKey } from "@workspace/api-client-react";
import { useQueryClient } from "@tanstack/react-query";
import { useToast } from "@/hooks/use-toast";
import { ArrowLeft, Save, Loader2 } from "lucide-react";
import { Link } from "wouter";

type FormValues = z.infer<typeof createResellerSchema>;

export default function AdminCreateReseller() {
  const [, setLocation] = useLocation();
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const createMutation = useAdminCreateReseller();
  ...
  return (
    <AppLayout role="admin" title="Add New Reseller">
      <div className="mb-6">
      ...
      <div className="bg-card border border-border rounded-2xl shadow-xl shadow-black/10 overflow-hidden max-w-4xl">
        <div className="p-6 border-b border-border/50 bg-secondary/20">
          <h2 className="text-xl font-display font-semibold">Reseller Information</h2>
          <p className="text-sm text-muted-foreground mt-1">Enter the details for the new reseller account. They will use the email and password to log in.</p>
        </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="p-6 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4 md:col-span-2">
              <h3 className="text-sm font-bold text-primary uppercase tracking-wider">Account Credentials</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">Email Address *</label>
                  <input {...form.register("email")} type="email" className="w-full px-4 py-2.5 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary/50 outline-none transition-all" placeholder="admin@reseller.com" />
                  {form.formState.errors.email && <p className="text-xs text-destructive">{form.formState.errors.email.message}</p>}
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">Initial Password *</label>
                  <input {...form.register("password")} type="password" className="w-full px-4 py-2.5 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary/50 outline-none transition-all" placeholder="••••••••" />
            ...
            <div className="space-y-4 md:col-span-2 pt-4 border-t border-border/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1.5">
                  ...
                  <input {...form.register("companyName")} className="w-full px-4 py-2.5 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary/50 outline-none transition-all" placeholder="Acme Telecoms" />
                ...
                <div className="space-y-1.5">
                  ...
                  <input {...form.register("contactName")} className="w-full px-4 py-2.5 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary/50 outline-none transition-all" placeholder="John Doe" />
            ...
            <div className="space-y-4 md:col-span-2 pt-4 border-t border-border/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-1.5">
                  ...
                  <input {...form.register("city")} className="w-full px-4 py-2.5 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary/50 outline-none transition-all" placeholder="Cape Town" />
                </div>
                <div className="space-y-1.5">
          ...
          <div className="pt-6 border-t border-border/50 flex justify-end gap-4">
            <Link href="/admin/resellers" className="px-6 py-2.5 rounded-xl font-medium border border-border hover:bg-secondary transition-colors">
              Cancel
            </Link>
            <button
              type="submit"
